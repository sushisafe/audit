...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        // Perform another deposit to make sure some SUSAFEs are minted in that 1 block.
        await this.chef.deposit(0, '100', {from: minter});
        assert.equal((await this.susafe.totalSupply()).valueOf(), '110');
        assert.equal((await this.susafe.balanceOf(minter)).valueOf(), '100');
        assert.equal((await this.susafe.balanceOf(dev)).valueOf(), '10');
        // Transfer ownership to timelock contract
        this.timelock = await Timelock.new(alice, time.duration.days(2), {from: alice});
        this.gov = await GovernorAlpha.new(this.timelock.address, this.susafe.address, alice, {from: alice});
